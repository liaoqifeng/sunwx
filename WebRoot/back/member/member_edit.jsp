...
<body>
	<div id="mainBox" class="main-box">
		<div id="mainPanel" class="easyui-panel" iconCls="icon-edit" title="&nbsp;&nbsp;修  改  会  员">
			<form id="editForm" action="${path }/back/member/edit.shtml" method="post">
		    	<input type="hidden" name="id" value="${member.id }"/>
		    	<ul id="tab" class="tab">
					<li><input value="基本信息" type="button" /></li>
					<li style="display: none;"><input value="个人资料" type="button" /></li>
					<li><input value="账户调整" type="button" /></li>
				</ul>
				<table width="100%" class="input tabContent table">
		    		...
		    		<tr>
		    			<td height="30" align="right">&nbsp;帮派：&nbsp;&nbsp;</td>
		    			<td align="left">
		    				<select name="faction.id">
		    					<c:if test="${factions != null}">
		    						<c:forEach var="row" items="${factions}">
		    						<option value="${row.id }" ${member.faction.id == row.id ? 'selected="selected"' : '' }>${row.name }</option>
		    						</c:forEach>
		    					</c:if>
		    				</select>
		    			</td>
	   			...
	   			<table width="100%" class="input tabContent table" style="display: none;">
		    		<tr style="display: none;">
	    		...
	    		<table width="100%" class="input tabContent table">
	    			...
	    			<tr>
		    			<td height="30" align="right">&nbsp;当前金币：&nbsp;&nbsp;</td>
		    			<td align="left"><font color="red">${member.score }</font></td>
		    		</tr>
		    		<tr>
		    			<td align="right">&nbsp;调整金币：&nbsp;&nbsp;</td>
		    			<td align="left"><input type="text" name="modifyScore" title="正数增加金币,负数减少金币" class="text middle tip" maxlength="10"/></td>
		    		</tr>
	    		</table>
