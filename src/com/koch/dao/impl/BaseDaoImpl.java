...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public T get(String propertyName, Object value) {
		...
		T t = null;
		try {
			List<T> list=  entityManager.createQuery(hql).setParameter(1, value).getResultList();
	...
	public T get(Map<String, Object> map) {
		Assert.notNull(map, "map must not be empty");
		...
		try {
			...
			// t = (T) entityManager.createQuery(hql).setParameter(1,
	...
	public List<T> getList(Map<String, Object> map) {
		return getList(map, null);
	}

	@SuppressWarnings("unchecked")
	public List<T> getList(Map<String, Object> map, OrderBy order) {
		Assert.notNull(map, "map must not be empty");
...
//		System.out.println(hql);
	...
	public Serializable getIdentifier(T entity) {
		...
		return (Serializable) this.entityManager.getEntityManagerFactory().getPersistenceUnitUtil()
	...
	private Root<T> getRoot(CriteriaQuery<?> criteriaQuery, Class<T> paramClass) {
		if ((criteriaQuery != null) && (criteriaQuery.getRoots() != null) && (paramClass != null)) {
	...
	private void setParameter(CriteriaQuery<T> criteriaQuery, List<Filter> paramList) {
		...
		Predicate predicate = criteriaQuery.getRestriction() != null ? criteriaQuery.getRestriction() : criteriaBuilder
				.conjunction();
				...
				if ((filter.getOperator() == Filter.Operator.eq) && (filter.getValue() != null)) {
					if ((filter.getIgnoreCase() != null) && (filter.getIgnoreCase().booleanValue())
						...
						predicate = criteriaBuilder.and(predicate, criteriaBuilder.equal(
								criteriaBuilder.lower(root.get(filter.getProperty())),
					...
					if ((filter.getIgnoreCase() != null) && (filter.getIgnoreCase().booleanValue())
							&& ((filter.getValue() instanceof String)))
						predicate = criteriaBuilder.and(predicate, criteriaBuilder.notEqual(
								criteriaBuilder.lower(root.get(filter.getProperty())),
								((String) filter.getValue()).toLowerCase()));
					else
						predicate = criteriaBuilder.and(predicate,
					...
					predicate = criteriaBuilder.and(predicate,
						...
						&& ((filter.getValue() instanceof String)))
					predicate = criteriaBuilder.and(predicate, criteriaBuilder.like(root.get(filter.getProperty()), "%"
							+ (String) filter.getValue() + "%"));
				else if ((filter.getOperator() == Filter.Operator.in) && (filter.getValue() != null))
					predicate = criteriaBuilder.and(predicate,
							root.get(filter.getProperty()).in(new Object[] { filter.getValue() }));
	...
	protected Long count(CriteriaQuery<T> paramCriteriaQuery, List<Filter> paramList) {
		...
		return (Long) this.entityManager.createQuery(criteriaQuery).setFlushMode(FlushModeType.COMMIT)
				.getSingleResult();
	}
	...
	@SuppressWarnings("unchecked")
	protected List<T> findList(CriteriaQuery<T> paramCriteriaQuery, Integer first, Integer count, List<Filter> filters,
			List<OrderBy> orders) {
		Assert.notNull(paramCriteriaQuery);
		Assert.notNull(paramCriteriaQuery.getSelection());
		Assert.notEmpty(paramCriteriaQuery.getRoots());
		...
		if (paramCriteriaQuery.getOrderList().isEmpty())
			paramCriteriaQuery.orderBy(new javax.persistence.criteria.Order[] { localCriteriaBuilder.desc(root
					.get("createDate")) });
		TypedQuery typedQuery = this.entityManager.createQuery(paramCriteriaQuery).setFlushMode(FlushModeType.COMMIT);
		if (first != null)
	...
	private void translationForm(From<?, ?> paramFrom1, From<?, ?> paramFrom2) {
		...
		while (localIterator.hasNext()) {
			...
			localObject2 = paramFrom2.join(((Join) localObject1).getAttribute().getName(),
					((Join) localObject1).getJoinType());
	...
	protected Integer getCount(CriteriaQuery<T> criteriaQuery) {
		...
		return Integer.valueOf(this.entityManager.createQuery(localCriteriaQuery).setFlushMode(FlushModeType.COMMIT)
	...
	public Pager<T> findByPager(CriteriaQuery<T> criteriaQuery, Pager<T> pager) {
		...
		if (StringUtils.isNotEmpty(orderBy) && orderType != null) {
			if (orderType == OrderType.asc) {
				criteriaQuery
						.orderBy(new javax.persistence.criteria.Order[] { criteriaBuilder.asc(root.get(orderBy)) });
			} else {
				criteriaQuery
						.orderBy(new javax.persistence.criteria.Order[] { criteriaBuilder.desc(root.get(orderBy)) });
			}
		}
		Integer l = getCount(criteriaQuery);
		int i = (int) Math.ceil(l * 1.0 / pager.getPageSize());
